...
// 初始化网格Grid状态
void mapCartesianGrid(PointCloud<PointXYZI>::Ptr elevatedCloud, array<array<int, numGrid>, numGrid>& cartesianData)
{
  ...
  // 然后所有相邻的相邻像元（即x-1，y  + 1，x，y +1，x +1，y +1 x -1，y，x +1，y，x -1，y -1，x，检查y − 1，x + 1，y +
  // 1）的占用状态，并用当前集群ID标记。 对m×n网格中的每个x，y重复此过程，直到为所有非空群集分配了ID。
...
void search(array<array<int, numGrid>, numGrid>& cartesianData, int clusterId, int cellX, int cellY)
{
  ...
  {                       // kernelSize = 3;循环3次
    ...
    if ((cellX + kXI) < 0 || (cellX + kXI) >= numGrid)
      continue;
    for (int kY = 0; kY < kernelSize; kY++)
    {
      int kYI = kY - mean;
      if ((cellY + kYI) < 0 || (cellY + kYI) >= numGrid)
        continue;
                        ...
                        std::vector<pcl::PointIndices>& cluster_indices)
{
  ...
  {
    if (cluster_seed_[id].get_clusterdPointNum() < minClusterdNum)
      continue;
    pcl::PointIndices r;
...
void componentClustering(PointCloud<pcl::PointXYZI>::Ptr elevatedCloud, std::vector<pcl::PointIndices>& cluster_indices)
{
  ...
  getClusteredPoints(elevatedCloud, cartesianData, cluster_seed_, numCluster,
                     cluster_indices);  // 依据聚类ID，将每个点存入对应的聚类 cluster_indices
}
